...
arguments = " %s "
getenv = True
log = /usr1/%s/condorLogs/pointy-Poisson_doitall/$(Cluster)-$(Process).log
...
### figure out whoami
whoami = sp.Popen(["whoami"], stdout=sp.PIPE).communicate()[0]
...
args = " $(gpsTarget) -o %s -t %s -s %d -S %d --pointybin %s -w %.6f -W %.6f -e %.6f --fmin %.6f --fmax %.6f --durmin %.6f --durmax %.6f -p %.6f -O %s/$(JobID)/ --signifmax %.3e --signifmin %s"%(opts.observatory, opts.frame_type, opts.kwstride, opts.framestrice, opts.pointybin, opts.coinc_window, opts.signif_window, opts.exclude, opts.fmin, opts.fmax, opts.durmin, opts.durmax, opts.pvalue, opts.output_dir, opts.signifmax, " --signifmin ".join(opts.signifmin))
if opts.verbose:
    args = " -v" + args
if opts.unsafe:
    args = args + " -u %s"%(opts.unsafe)

if opts.verbose:
    print "writing %s"%(subfile)
subfile_obj = open(subfile, "w")
subfile_obj.write( sub_header%(opts.universe, opts.pointybin, args, whoami, opts.output_dir, opts.output_dir, opts.accounting_group, opts.accounting_group_user) )
subfile_obj.close()
...
for ind, gps in enumerate(args):
    ...
    print >> dagfile_obj, "VARS  %s JobID=\"%s\" gpsTarget=\"%.9f\""%(jobID, gps)
